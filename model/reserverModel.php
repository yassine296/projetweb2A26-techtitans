...
public static function delete($id) {
    $sql = "DELETE FROM reservation_bagage WHERE id = :id";
    ...
    try {
        ...
        error_log('Erreur dans delete reservation: ' . $e->getMessage());
        return false;
    }
}
...
public static function update($id, $nb_places, $type_bagage, $prix_total) {
    $sql = "UPDATE reservation_bagage 
    ...
    try {
        ...
        return $query->rowCount() > 0; // Retourne true uniquement si des lignes sont modifiées
    } catch (Exception $e) {
        error_log('Erreur dans update reservation: ' . $e->getMessage());
        return false;
    }
}
...
public static function getReservationById($id) {
    $sql = "SELECT * FROM reservation_bagage WHERE id = :id";
    $db = config::getConnexion();
    try {
        $query = $db->prepare($sql);
        $query->execute(['id' => $id]);
